    ...
    for batch_i, batch in enumerate(data_loader):
        ...
        if model_name =='radialog':
            ...
            if 'qwen' in model_name.lower():
                ...
                if batch_i < 3:
                    print(f"DEBUG - Raw Qwen output: {generated_text[:200]}...")
            else:
                generated_text, _ = inference_llavaov(model, processor, image_path, prompt)

        # Handle R1 mode: extract answer from structured tags
        if r1:
            match_pattern = r'<answer>(.*?)</answer>'
            m = re.search(match_pattern, generated_text, re.DOTALL | re.IGNORECASE)
            if m:
                ...
                if batch_i < 3:
                    print(f"DEBUG - No <answer> tags found, using raw output")
